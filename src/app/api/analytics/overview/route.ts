...
export async function GET(request: NextRequest) {
  ...
  const totalLinks = queryOne<{ count: number }>("SELECT COUNT(*) as count FROM links WHERE user_id = ?", session.userId)?.count ?? 0;

  const totalClicks = queryOne<{ total: number }>(
    "SELECT COALESCE(SUM(l.clicks), 0) as total FROM links l WHERE l.user_id = ?",
    session.userId
  )?.total ?? 0;

  const today = new Date().toISOString().slice(0, 10);
  const clicksToday = queryOne<{ count: number }>(
  ...
  const clicksThisWeek = queryOne<{ count: number }>(
    "SELECT COUNT(*) as count FROM link_clicks lc JOIN links l ON lc.link_id = l.id WHERE l.user_id = ? AND lc.created_at >= ?",
    session.userId, weekAgo
  )?.count ?? 0;

  const topLink = queryOne<{ title: string; clicks: number }>(
