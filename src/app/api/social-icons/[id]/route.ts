...
export async function DELETE(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const icon = await queryOne<SocialIconRow>("SELECT * FROM social_icons WHERE id = ? AND user_id = ?", iconId, session.userId);
  if (!icon) return jsonError("Social icon not found", 404);

  await execute("DELETE FROM social_icons WHERE id = ?", iconId);
